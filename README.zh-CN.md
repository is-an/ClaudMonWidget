...
这个状态应该只是一瞬。轮询一旦发现窗口已经翻页，就当场向 Anthropic 索取新窗口，
而不是等下一次定时同步，空档因此是几秒而不是 `syncSeconds`。
...
- **`return $数组` 会在管道里被展开。** 调用方拿到的是装箱元素的 `object[]`，不是要的
  ...
  要写成 `, $数组`。
- **挂到事件上的脚本块会按位置收到 `(sender, args)`。** 如果给它加了带类型的参数，
  转换的就是 sender（比如一个 `DispatcherTimer`），异常在处理器内部抛出，而且不会
  显现到任何地方。auto sync 和 `Sync now` 就是这样双双失效的，而小组件看上去一切
  正常 —— 缓存停止刷新，5 小时窗口翻页，百分比一直灰着直到重启，因为重启走的是直接
  调用的路径。处理器不带参数，真正的实参由外层包装脚本块传入。`test-menu.ps1`
  两边都钉住了。
